...
class UserController extends Controller
{
    ...
    public function view($id){
        ...
        return view('/search.facultyMember', compact('user'));
    ...
    public function credit($id){
        ...
        $credUser = User::find($id);
        
        $credUser->roles()->attach(Role::where('title', 'Credit')->get());
    
        session()->flash('confirmation', 'Successfully added credential officer!');
        return view('/users/credentialManPage', compact('user'));
        
    }
    
    public function deleteCredit($id){
        
        $credDelete = User::find($id);
      
        $credDelete->roles()->detach(Role::where('title', 'Credit')->get());
        
        session()->flash('confirmation', 'Successfully removed credential officer!');
        return view('/users/credentialManPage', compact('user'));
        
    }
    
    public function changeCred($id){
    
        $credChange = User::find($id);
    
        $newCred = request('titleCred');
        $credChange->roles()->attach(Role::where('title', compact('newCred'))->get());
    
        session()->flash('confirmation', 'Successfully updated credentials!');
        return redirect('/users/credAppal');
